...
export default {
  ...
  beforeMount(){
    //跳转到search后,合并保留query和params的参数到searchParams中,每一次跳转清除三级id,避免有多个id
  ...
  computed:{
    ...mapGetters(['goodsList']),   //搜素商品后返回的商品内容数组
    // 判读是综合还是价格排序
    isOne(){           
  ...
  methods:{
    ...
    changOrder(flag){
      ...
      let originSort = this.searchParams.order.split(':')[1]  //asc//desc
      let newOrder;
      if(flag==originFlag){
  ...
  watch:{
    $route(newvalue,oldvalue){
      //保留query和params参数，把query和params参数合并到searchParams
      Object.assign(this.searchParams,this.$route.query,this.$route.params);
      this.getData();
    }
  }
